...
def api_activity():
    """Return the last 50 attendance events (last 48 hours)."""
    rows = execute_query(
        "SELECT a.telegram_id, e.name, a.status, a.date, a.marked_at "
        "FROM attendance a JOIN employees e ON a.telegram_id = e.telegram_id "
        "WHERE a.marked_at >= NOW() - INTERVAL '48 hours' "
        "ORDER BY a.marked_at DESC LIMIT 50",
        fetch_all=True
    )
    events = []
    for r in rows:
        ...
        if isinstance(ma, datetime):
            if ma.tzinfo is None:
                ma = ma.replace(tzinfo=ZoneInfo("UTC"))
            ma = ma.astimezone(IST).isoformat()
        d = r["date"]
...
@app.route("/api/employees/archive", methods=["POST"])
def api_archive_employee():
    data = request.json or {}
    password = data.get("password")
    if password != ADMIN_PASSWORD:
        return jsonify({"error": "Unauthorized"}), 401
...
@app.route("/api/employees/restore", methods=["POST"])
def api_restore_employee():
    data = request.json or {}
    password = data.get("password")
    if password != ADMIN_PASSWORD:
        return jsonify({"error": "Unauthorized"}), 401
